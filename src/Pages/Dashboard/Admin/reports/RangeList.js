...
export default function RangeOrderList() {
  const [Order, fetchOrder] = useState([])
  const [Orderstate, fetchOrderstate] = useState(false)
  
  const rangeSearch = (event) => {
    event.preventDefault()
    let from_date = event.target.from_date.value
    let to_date = event.target.to_date.value
    axios
      .get(
        `${API}/api/order_range_filter/?from_date=${from_date}&to_date=${to_date}`,
      )
      .then((res) => {
        if (res.status === 200) {
  ...
  return (
    <div>
      <div className=" scroll md:mt-16 justify-center w-full min-h-screen">
        ...
        <p className="uppercase font-bold">Filter By From Date and To Date</p>
          <div className="py-4">
